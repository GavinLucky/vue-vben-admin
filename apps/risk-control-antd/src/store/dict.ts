...
export function dictToOptions(data: RgApi.Dict.IDictData[]): Option[] {
...
export const useDictStore = defineStore('app-dict', () => {
   ...
   * 一般是dictTag使用
   */
  const dictMap = reactive(new Map<string, RgApi.Dict.IDictData[]>());
  /**
  ...
  function getDict(dictName: string): RgApi.Dict.IDictData[] {
    if (!dictName) return [];
    // 没有key 添加一个空数组
   ...
   *
   */
  function setDictInfo(dictName: string, dictValue: RgApi.Dict.IDictData[]) {
